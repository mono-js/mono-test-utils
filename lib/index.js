...
exports.start = async (dir) => {
	...
	// Start mono
	const { app, server } = await mono()
	// Flush logs output
	const { stdout, stderr } = stdMocks.flush()
	stdMocks.restore()
	return { app, server, stdout, stderr }
}

exports.close = async (server) => {
	if (!server) throw new Error('No server provided to close(server)')
	await cb(server.close.bind(server))
}

const url = (path) => `http://localhost:${context.conf.mono.http.port}` + join('/', (path || ''))
